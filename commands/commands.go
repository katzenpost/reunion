...
const (
	...
	sendT1Length          = cmdOverhead + 8 + reunion.Type1MessageSize
	sendT2Length          = cmdOverhead + 8 + 32 + 32 + reunion.Type2MessageSize
	sendT3Length          = cmdOverhead + 8 + 32 + 32 + reunion.Type3MessageSize
...
type FetchState struct {
	...
	T1Hash [primitives.HashSize]byte
}
...
func (s *SendT2) ToBytes() []byte {
	...
	copy(out[9+primitives.HashSize:], s.DstT1Hash[:])
	out = append(out, s.Payload...)
	return out
}
...
func sendT2FromBytes(b []byte) (Command, error) {
	...
	s := new(SendT2)
	s.Epoch = binary.BigEndian.Uint64(b[1:9])
	srchash := [primitives.HashSize]byte{}
	copy(srchash[:], b[9:9+primitives.HashSize])
	s.SrcT1Hash = srchash
	dsthash := [primitives.HashSize]byte{}
	copy(dsthash[:], b[9+primitives.HashSize:9+primitives.HashSize+primitives.HashSize])
	s.DstT1Hash = dsthash
	s.Payload = b[9+primitives.HashSize+primitives.HashSize:]
	return s, nil
}
