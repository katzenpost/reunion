...
func TestFetchStateCommand(t *testing.T) {
	...
	cmd := new(FetchState)
	cmd.Epoch = 1234
	cmd.T1Hash = [primitives.HashSize]byte{}
...
func TestSendT1Command(t *testing.T) {
	...
	cmd := new(SendT1)
	cmd.Epoch = 123
	cmd.Payload = make([]byte, reunion.Type1MessageSize)
...
func TestSendT2Command(t *testing.T) {
	...
	cmd := new(SendT2)
	cmd.Epoch = 123
	cmd.SrcT1Hash = [primitives.HashSize]byte{}
	fillRand(require, cmd.SrcT1Hash[:])

	cmd.DstT1Hash = [primitives.HashSize]byte{}
	fillRand(require, cmd.DstT1Hash[:])
	cmd.Payload = make([]byte, reunion.Type2MessageSize)
